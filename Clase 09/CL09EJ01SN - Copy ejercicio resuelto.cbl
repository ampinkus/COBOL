      ...
      * reports that the end of the file has been reached.
           OPEN INPUT ENTRADA.
           EVALUATE FS-ENTRADA *> Evalua las condicones de error al abrir el archivo
               WHEN '00'
                    PERFORM 1500-LEER-ARCHIVO *> Todo bien, voy a leer el archivo
                       THRU 1500-LEER-ARCHIVO-EXIT
               WHEN '35'
                    SET WS-ENTRADA-EOF-YES TO TRUE *> Archivo no existe, imprimo error
                    DISPLAY 'NO SE ENCUENTRA EL ARCHIVO DE ENTRADA'
                    DISPLAY 'FILE STATUS: ' FS-ENTRADA
               WHEN OTHER
                    SET WS-ENTRADA-EOF-YES TO TRUE *> Falló la lectura ,  imprimo error
      ...
      *----------------------------------------------------------------*
       1500-LEER-ARCHIVO.
           ...
           EVALUATE FS-ENTRADA
               ...
               WHEN '10'
                    CONTINUE *> va al fin del EVALUATE pero se llego al final del archivo.
               WHEN OTHER
                    SET WS-ENTRADA-EOF-YES       TO TRUE
      ...
      * Vuelvo a leer el siguiente registro
